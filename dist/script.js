    ...
    setTimeout(function () {
      ...
      document.querySelectorAll('[data-modal]').forEach(function (item) {
        if (getComputedStyle(item).display !== 'none') {
          // Выдает стили скомпилированные браузером
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1; // Текуший слайд

  var paused;
  var items = document.querySelectorAll(slides); // Получим слайды

  function showSlides(n) {
    if (n > items.length) {
      // Если кол-во слайдов больше индекса 
      slideIndex = 1;
    }

    if (n < 1) {
      slideIndex = items.length; // Если меньше
    }
